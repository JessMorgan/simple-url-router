...
async def login_page(request: Request):
    """Display the login page. Returns 401 if user is not authenticated.
    
    This prevents unauthorized access to the login page itself and forces
    unauthenticated users to redirect to /login with a 401 response.
    """
    token = request.cookies.get("session")
    if not token or not verify_session_token(token):
        raise HTTPException(status_code=401, detail="Login page requires authentication")
